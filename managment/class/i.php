...
class I {	
	...
	public function listHouses(){
		...
		while ($house = $result->fetch_assoc()) { 				
			...
			$rows[] = $house['lieu'];			
			$rows[] = $house['image'];			
			$rows[] = '<button type="button" name="view" id="'.$house["id"].'" class="btn btn-info btn-xs view">
            <span class="glyphicon glyphicon-file" title="View"></span></button>';			
			$rows[] = '<button type="button" name="update" id="'.$house["id"].'" class="btn btn-warning btn-xs update">
            <span class="glyphicon glyphicon-edit" title="Edit"></span></button>';
			$rows[] = '<button type="button" name="delete" id="'.$house["id"].'" class="btn btn-danger btn-xs delete" >
...
if(isset($_POST['cherb'])){
	...
	$sqlQuery = "SELECT * FROM ".$this->houseTable." where lieu  like'%$cher%' ";
	...
	$stmtTotal = $this->conn->prepare("SELECT * FROM house where lieu  like'%$cher%'");
	$stmtTotal->execute();
	$allResult = $stmtTotal->get_result();
	$allRecords = $allResult->num_rows;
